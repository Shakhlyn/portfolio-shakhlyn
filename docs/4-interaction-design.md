...
### 5.2 Currently (`#current-role`)

Heading text is "Currently"; the anchor id is unchanged and stays a public hash. Static
card whose only interaction is the row of project links at its foot — router `Link`s to
`/projects/:slug`, no hover treatment on the card itself, which would be a false
affordance around real controls. Appearance in `docs/3-style-preference.md` §6.3, which
specifies **no eyebrow** on this section.

It carries the page's single availability sentence, moved here from About
(`docs/tickets/RV-tickets.md` RV-T01).

It reveals as one block on scroll (animation 2). Nothing inside it staggers — not the
stack badges, not the project links — per the closed-list rule in §8.

`#current-role` stays absent from `ANCHOR_SECTION_IDS`: it has no nav item, so the scroll
spy does not observe it. Unchanged by RV-T01.
...
**No closing line.** The `lookingFor` sentence that used to close this section moved to
§5.2, where an actionable sentence is four screens higher up the page
(`3-style-preference.md` §6.5, `docs/tickets/RV-tickets.md` RV-T01). About ends on its
last paragraph.
...
| 20  | Row 13 put the availability sentence at the bottom of About, four screens down                                   | **Moved to the Currently panel** as `CURRENT_ROLE.availability`, keeping row 13's treatment — `fg`, `border-l-2 border-accent`, `pl-4`. Row 13's own reasoning ("folded into the prose it disappears") applies just as well to the foot of the page. `AboutType.lookingFor` is deleted, so the page carries one availability sentence, not two. About ends on its last paragraph.                                                                                                                                                                                                                                                                                                                  | `3-style-preference.md` §6.3, §6.5 · this doc §5.2, §5.4 · `docs/tickets/RV-tickets.md` RV-T01                                    |
| 21  | Current Role's four scope bullets were each duplicated elsewhere on the same page                                | **The section takes a different job, not different copy.** Rebuilt as "Currently", a present-tense status panel: employer and placement, one sentence of current scope, stack, availability, and links to the projects built at the placement. All achievement claims are dropped — Projects makes them with more detail. Heading text changes; `id="current-role"` does not.                                                                                                                                                                                                                                                                                                                      | `3-style-preference.md` §6.3 · `2-architecture.md` §8 · this doc §5.2 · `docs/tickets/RV-tickets.md` RV-T01                       |
| 22  | Which projects the Currently panel links to                                                                      | **An explicit `projectSlugs` list, never derived from `category: 'professional'`.** `bhoganti-web-app` is earlier work for the same employer and is finished; deriving from the category would file completed work under a present-tense heading. Employer and placement are likewise two fields, not one.                                                                                                                                                                                                                                                                                                                                                                                         | `3-style-preference.md` §6.3 · this doc §5.2 · `docs/tickets/RV-tickets.md` RV-T01                                                |
| 23  | RV-T01 left the hero restating the panel below it — same role, same employer, same platform and modules          | **The hero slims to positioning only (RV-T02).** The current position line and its two `ProfileType` fields are deleted; the value proposition cuts to one sentence about how the author works rather than where. The employer now renders exactly once on the home page. Stale year counts ("2.5 years", "nearly 2 years") are removed rather than guessed — the author supplies a replacement.                                                                                                                                                                                                                                                                                                   | `3-style-preference.md` §6.2 · this doc §5.1 · `5-epic-list.md` E09 · [../tickets/RV-tickets.md](../tickets/RV-tickets.md) RV-T02 |
| 24  | The hero `h1`'s two-step type scale wrapped a 25-character name                                                  | **One fluid `--text-display-name` token, sized for the `split` layout's `lg` text column (RV-T03/RV-T04).** That column is the binding case, not the smallest phone: it narrows to 624px at exactly the width a two-step scale jumps to its largest step. Tuned for the widest face in the `system-ui` stack — the stack resolves per platform, and RV-T03's estimate against a narrower face wrapped at 1024px in the wild. `--text-display` and `--text-display-md` are untouched; the 404 glyph still uses them                                                                                                                                                                                 | `3-style-preference.md` §3.2, §6.2 · `docs/tickets/RV-tickets.md` RV-T03, RV-T04                                                  |
| 25  | The Currently panel rendered one date range against two organisations                                            | **Employer and placement each carry their own range, rendered beside themselves (RV-T05).** The employer's sits in the `h3` as a mono `body-sm` span, the placement's on the line below. A range on an unlabelled line of its own is attributed to whatever is nearest — the failure mode that let a seven-month backdating read as correct. Field renamed, not supplemented, so the fix is compile-checked                                                                                                                                                                                                                                                                                        | `3-style-preference.md` §6.3 · this doc §5.2 · `docs/tickets/RV-tickets.md` RV-T05                                                |

...
import { NOT_FOUND_METADATA } from '@/data/metadata';
 ...
 *
 * **`noindex` lives in the component, not in `router.tsx`.** This page renders
 * from two places — the router's `path: '*'` and `ProjectPage`'s early return —
 * and under an SPA fallback both answer `200` with the shell, so there is no
 * status code for a crawler to read. Putting the tag here covers both for free.
 */
export const NotFoundPage = (): ReactElement => {
  useDocumentMetadata(NOT_FOUND_METADATA);
